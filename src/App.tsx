  ...
  const filteredPlayers = useMemo(() => {
    const filtered = players.filter(p => {
      ...
      const matchesFilter = 
        ...
        p.status === posFilter ||
        (posFilter.startsWith("stars-") && p.stars === parseInt(posFilter.split("-")[1]));
                ...
                <div className="flex gap-0.5 mb-1">
                  {[...Array(5)].map((_, i) => (
                    <Star 
                      ...
                      className={cn(
                        "w-3 h-3", 
                        i < (p.stars || 3) ? "fill-yellow-400 text-yellow-400" : "text-black/10"
                      )} 
                    />
                  ))}
                </div>
                  ...
                  <div className="flex gap-1 no-print">
                    {[...Array(5)].map((_, i) => (
                      <Star 
                        ...
                        className={cn(
                          "w-5 h-5", 
                          i < (player.stars || 3) ? "fill-yellow-400 text-yellow-400" : "text-black/10"
                        )} 
                      />
                    ))}
                  </div>
